...
class TaskCategoryCacheManager extends GlobalCacheManager<taskCategory[]> {
   ...
   */
  private tasklastUpdate: GlobalCacheManager<{ [key: string]: Date }> =
    new GlobalCacheManager<{ [key: string]: Date }>("tasklastUpdate");
  constructor() {
  ...
  setTaskLastUpdate(positionOrCategoryID: number | string, date: Date) {
    const tasklastUpdate = {
      ...this.tasklastUpdate.get(),
      [positionOrCategoryID]: date,
    };
...
export class Task {
  ...
  async getTaskCategories() {
    try {
      ...
      let tasks = navigator.onLine
        ? await this.getTasksFromApi(this.accessToken)
        : await this.getTasksFromFile();
      if (!tasks) tasks = [];
      this.tasksCategoryList.set(
        tasks.map((taskCategory) => {
          // if category with same id exists, merge them
          const existingCategory = this.tasksCategoryList
            .get()
            ?.find((category) => category?.id === taskCategory?.id);
          if (existingCategory) {
            taskCategory.tasks = [
  ...
  async getTasksFromApi(accessToken: string): Promise<taskCategory[]> {
    ...
    const taskCategories: taskCategory[] = response.data.items.map(
      (item: any) => {
        return {
          id: item.id,
          name: item.title,
  ...
  async getTasksByCategoryPosition(position: number) {
    try {
      ...
      console.error(
        "Error getting tasks by category position:",
  ...
  async retrieveTasksByCategoryPosition(position: number): Promise<task[]> {
    ...
    const isCacheValid =
      (cacheLastUpdate &&
        new Date().getTime() - cacheLastUpdate.getTime() < 45 * 1000) ||
      !navigator.onLine;
  ...
  async retrieveTasksByCategoryID(categoryID: string): Promise<task[]> {
    const cacheLastUpdate =
      this.tasksCategoryList.getTaskLastUpdate(categoryID);
    const isCacheValid =
      cacheLastUpdate &&
      (new Date().getTime() - cacheLastUpdate.getTime() < 45 * 1000 ||
        !navigator.onLine);
  ...
  updateTaskCategoryTasks(categoryID: string, tasks: task[]) {
    const updatedTaskCategories = this.tasksCategoryList
      .get()
      .map((taskCategory) => {
  ...
  async getTaskByIdFromFile(categoryID: string): Promise<task[]> {
    ...
    return taskCategories.find(
      (taskCategory: taskCategory) => taskCategory.id === categoryID
    ).tasks;
  }
